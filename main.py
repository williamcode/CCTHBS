...
def main():
    ...
    for _ap_sql_re in ap_sql_re:
        sql.append("""insert into voucher values('ap','%s','%s',%8.2f,%8.2f,'','%s','','','','','%s','%s')"""%(_ap_sql_re[1],_ap_sql_re[3],_ap_sql_re[4],_ap_sql_re[5],_ap_sql_re[6],_ap_sql_re[2]))
    ...
    for _ar_sql_re in ar_sql_re:
        sql.append("""insert into voucher values('ar','%s','%s',%8.2f,%8.2f,'','%s','','','','','%s','%s')"""%(_ar_sql_re[1],_ar_sql_re[3],_ar_sql_re[4],_ar_sql_re[5],_ar_sql_re[6],_ar_sql_re[2]))
    ...
    for _bank_rec_sql_re in bank_rec_sql_re:
        sql.append("""insert into voucher values('br','%s','%s',%5.2f,%8.2f,'%s','%s','','%s','%s','%s','%s','%s')"""%(_bank_rec_sql_re[1],_bank_rec_sql_re[5],
                    _bank_rec_sql_re[4],_bank_rec_sql_re[3],_bank_rec_sql_re[2],_bank_rec_sql_re[0],str(_bank_rec_sql_re[8]),_bank_rec_sql_re[7],_bank_rec_sql_re[9],_bank_rec_sql_re[6],_bank_rec_sql_re[11]))
        # type,日期，币种，手续费，银行金额，收款银行，账套，‘’，金额（字符型），客户,收款币种，合同号，b.excelserverrcid


    bank_pay_sql = """select payer,cno,收款人,case when 币种='CNY' then '' else 币种 end as cur,case when 币种='CNY' then 0 else 付款金额 end as famt,
                    case when 币种='CNY' then  付款金额 else 0 end as amt,支付时间,支付银行   from 付款明细OA_excel where month(支付时间)=%d and year(支付时间)=%d """

    bank_pay_sql_re = msquery.Sqlexe(bank_pay_sql%(m,y))

    for _bank_pay_sql_re in bank_pay_sql_re:
        sql.append("""insert into voucher values('bp','%s','%s',%8.2f,%8.2f,'%s','%s','','%s','','','%s','')"""%(_bank_pay_sql_re[6],_bank_pay_sql_re[3],
                    _bank_pay_sql_re[4],_bank_pay_sql_re[5],_bank_pay_sql_re[2],_bank_pay_sql_re[0],_bank_pay_sql_re[7],_bank_pay_sql_re[1]))
